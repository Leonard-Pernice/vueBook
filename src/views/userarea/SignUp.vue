      ...
      <form @submit.prevent="submitForm">
        <div class="mt-2">
          <label class="block text-white text-sm mb-2" for="email">Email Address</label>
            <input id="email" type="email" class="shadow appearance-none border rounded w-full h-10 py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" v-model="userdata.username" required>
        </div>
        <div class="mt-2">
          <label class="block text-white text-sm mb-2" for="password">Password</label>
          <div class="relative">
            <input v-if="showPassword" id="password" type="text" class="shadow appearance-none border rounded w-full h-10 py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" v-model="userdata.password">
            <input v-else type="password" class="shadow appearance-none border rounded w-full h-10 py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" v-model="userdata.password">
            <button class=" absolute right-0 bg-blue-500 hover:bg-blue-700 text-white h-10 font-bold py-2 px-4 rounded-r focus:outline-none focus:shadow-outline" type="button" @click="toggleShowPassword">
        ...
        <div class="mt-2">
          <label class="block text-white text-sm mb-2" for="password">Repeat password</label>
          <input v-if="showPassword" type="text" class="shadow appearance-none border rounded w-full h-10 py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" v-model="userdata.password2">
          <input v-if="!showPassword" type="password" class="shadow appearance-none border rounded w-full h-10 py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" v-model="userdata.password2">
        </div>
        <div class="bg-red-500 text-white p-4 rounded-md" v-if="userdata.errors.length">
          <p v-for="error in errors" :key="error">{{ error }}</p>
        </div>
        <div class="mt-2">
          <button class="bg-green-400 hover:bg-green-500 text-white font-bold py-2 px-4 mb-2 rounded-md w-full">Sign up</button>
        </div>
        <hr>
        <div class="block text-white text-sm mt-4" for="password">
          Or <RouterLink to="/login"><span class="text-blue-400">click here</span></RouterLink> to log in!
        </div>
      </form>
    </div>
  </div>
